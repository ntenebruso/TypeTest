    ...
    methods: {
        ...
        handleKeyDown(e) {
            ...
            if (
                ...
                !this.ignoredKeycodes.find((code) => code == e.keyCode)
            ) {
                this.startTest();
            }
            ...
            if (e.keyCode == 32) {
                ...
                if (this.currentLetterElementIndex > 0) {
                    ...
                    if (this.currentLetterElementIndex == 0) {
                        ...
                        this.moveCaret(
                            letters[0].offsetLeft + letters[0].offsetWidth
                        );
                    } else {
                        this.moveCaret(
                            letters[this.currentLetterElementIndex - 1]
                                .offsetLeft +
                                letters[this.currentLetterElementIndex - 1]
                                    .offsetWidth
                        );
                    }
                    letters[this.currentLetterElementIndex].classList.remove(
                        "correct"
                    );
                    letters[this.currentLetterElementIndex].classList.remove(
                        "incorrect"
                    ...
                    if (
                        letters[
                            this.currentLetterElementIndex
                        ].classList.contains("extra")
                    ) {
                        letters[
                            this.currentLetterElementIndex
                        ].parentNode.removeChild(
                            letters[this.currentLetterElementIndex]
                        );
                    }
                    this.inputValue.pop();
                } else {
                ...
                if (this.inputValue.length > letters.length) {
                    wordElement[
                        this.currentWordElementIndex
                    ].innerHTML += `<span class="incorrect extra">${this.keyPressed}</span>`;
                    letters = Array.from(
                        wordElement[this.currentWordElementIndex].children
                    ...
                    letters[this.currentLetterElementIndex].classList.add(
                        "correct"
                    );
                    this.correctChars++;
                } else if (
                ...
                this.moveCaret(
                    letters[this.currentLetterElementIndex].offsetLeft +
                        letters[this.currentLetterElementIndex].offsetWidth
                );
        ...
        moveCaret(xPos) {
            gsap.to(".caret", { x: xPos, duration: 0.1 });
        },
    },
    computed: {
        words() {
